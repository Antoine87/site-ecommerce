...
class ClientDAO implements IClientDAO {
    ...
    */
    ...
    private function insert(ClientDTO $client){
        ...
        $statement = $this->pdo->prepare($sql);
        $statement->execute([
            $client->getNom(), $client->getPrenom(), $client->getEmail(), $client->getMotDePasse(), $client->getDateNaissance()
        ]);
    }

    private function update(ClientDTO $client){
        $sql = "UPDATE clients SET nom=? , prenom=? , email=? , mot_de_passe=? , date_naissance=?  WHERE id_client=? ";
        $data = array(
            $client->getNom(),
$client->getPrenom(),
$client->getEmail(),
